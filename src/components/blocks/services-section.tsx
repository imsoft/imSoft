...
export function ServicesSection({ dict, lang, services }: ServicesSectionClientProps) {
  ...
  const technologyServices = displayServices.filter(s => s.category === 'technology')
  const marketingServices = displayServices.filter(s => s.category === 'marketing')

  const sectionTitle = dict.services.title
  const sectionSubtitle = dict.services.subtitle
  const marketingTitle = lang === 'en' ? 'Marketing Services' : 'Servicios de Marketing'
  const marketingSubtitle = lang === 'en'
    ? 'Strategic marketing solutions to strengthen your brand and connect with your audience'
    ...
    <section id="services" className="py-16 md:py-24 bg-background">
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        ...
        {marketingServices.length > 0 && (
          <>
            <ScrollReveal className="text-center mb-12 mt-24" direction="up">
              <h2 className="text-4xl md:text-5xl font-bold mb-4">
                {marketingTitle}
              </h2>
              <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
