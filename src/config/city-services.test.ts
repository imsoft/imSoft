...
import { CITY_SERVICES, CITY_SERVICE_SLUGS, cityServiceContent, cityServiceHref, cityServicePages, cityServiceTitle } from './city-services';
...
describe('landings de ciudad + servicio', () => {
  it('existen las cuatro de Guadalajara y se resuelven por ruta', () => {
    expect(paginas.map((p) => p.slug).sort()).toEqual([...CITY_SERVICE_SLUGS].sort());
    ...
    expect(cityServiceContent('monterrey', 'paginas-web')).toBeNull();
  ...
  it('cada pagina nombra la ciudad en titulo, h1 y descripcion, y la descripcion cabe en Google', () => {
    for (const { c } of paginas) {
      expect(c.seoTitle).toContain('Guadalajara');
      expect(c.h1).toContain('Guadalajara');
      expect(c.seoDescription).toContain('Guadalajara');
  ...
  it('los proyectos citados existen en el portafolio y no se les atribuye ciudad', () => {
    ...
    for (const { c } of paginas) {
      ...
      for (const item of c.proof.items) {
        ...
        expect(item.description.toLowerCase()).not.toMatch(/en guadalajara|de guadalajara|en zapopan/);
      }
    }
  });
  ...
  it('los precios "desde" son los publicados en el sitio', () => {
    ...
    expect(CITY_SERVICES.guadalajara['tiendas-en-linea'].pricing).toBeUndefined();
  });
